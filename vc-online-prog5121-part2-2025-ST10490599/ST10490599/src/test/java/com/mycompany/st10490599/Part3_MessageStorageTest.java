...
public class Part3_MessageStorageTest {
    ...
    public Part3_MessageStorageTest() {
        // no System.in globally set here, set in tests that need it
    }
    ...
    public void setup() {
        ...
        testDataStore.put("HASH3", new Part2_MessageData("msgid3", "HASH3", "+27834557896", "Where are you? You are late! I have asked you to be on time.", "+27123456789"));
        testDataStore.put("HASH4", new Part2_MessageData("msgid4", "HASH4", "0838884567", "Ok, I am leaving without you.", "+27123456789"));

        Part3_MessageStorage.populateMessages(testDataStore);
    }
    ...
    public void teardown() {
        ...
        outContent.reset();
        Part3_MessageStorage.populateMessages(new HashMap<>());
        System.setIn(System.in); // Reset System.in to original if changed
    }
    ...
    public void testSentMessagesArrayCorrectlyPopulated() {
        ...
        // Count non-null messages in sentMessages
        long count = Arrays.stream(Part3_MessageStorage.sentMessages)
                           .filter(msg -> msg != null)
                           .count();
        assertEquals(4, count, "sentMessages should contain 4 messages.");

        // Check all expected messages are present somewhere
        ...
        // Check senderPhones contains expected sender
    ...
    public void testSearchForMessageID() {
        ...
        System.setIn(new ByteArrayInputStream((searchID + System.lineSeparator()).getBytes()));
        outContent.reset();

        Part3_MessageStorage.searchByID();
    ...
    public void testSearchByRecipient() {
        ...
        System.setIn(new ByteArrayInputStream((recipient + System.lineSeparator()).getBytes()));
        outContent.reset();

        Part3_MessageStorage.searchByRecipient();

        String output = outContent.toString();
        assertTrue(output.contains("It is dinner time!"), "Output should contain message 'It is dinner time!'");
        // Your data has "Where are you?" recipient +27834557896 so we won't check for that here
    ...
    public void testDeleteByHash() {
        String hashToDelete = "HASH3"; // Corresponds to "Where are you?..."
        System.setIn(new ByteArrayInputStream((hashToDelete + System.lineSeparator()).getBytes()));
        outContent.reset();

        Part3_MessageStorage.deleteByHash();

        String output = outContent.toString();
        assertTrue(output.contains("Deleting"), "Output should confirm deletion.");

        // Verify message was deleted by searching for it again
        System.setIn(new ByteArrayInputStream((hashToDelete + System.lineSeparator()).getBytes()));
        outContent.reset();
    ...
    public void testDisregardMessage() {
        ...
        // Fill disregardMessages to test full condition
        for (int i = 0; i < 8; i++) {
